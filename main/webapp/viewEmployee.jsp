...
<body>
	...
	<table border="1px" width="50%">
	    ...
	    for(Address address : addresses) { %>
	        <tr>
	            ...
	            <td><%= address.getIsDeleted() %></td>
	            <% i++; %>
	            <td><a href = "employee?action=editAddress&option=<%= i - 1 %>&id=<%= employee.getId() %>">
	                <button>Edit</button></a></td>
	            <td><a href = "employee?action=deleteAddress&option=<%= i - 1 %>&id=<%= employee.getId() %>">
	                <button>Delete</button></a></td>
	        </tr>
	    <% } %>
	</table>
	<br><br>
	<a href = "employee?action=newAddress&id=<%= employee.getId() %>">Add new address</a>
	<% } %>
	<% if (null != request.getAttribute("idTo")) { %>
	<br><br>
	<form action="employee?action=insertAddress&id=<%= employee.getId() %>" method="post">
    ...
    <a href = "employee?action=view&id=<%= employee.getId() %>"><button>Cancel</button></a>
	<% } %>
	<br><br><br>
	<% if (0 != projects.size()) { %>
	<table border="1px" width="50%">
	    ...
	    <% for(Project project : projects) { %>
	        <tr>
	            ...
	            <td><a href = "employee?action=unassign&projectId=<%= project.getId() %>&employeeId=<%= employee.getId() %>">
	                <button>Unassign</button></a></td>
	        </tr>
	    <% } %>
	</table>
	<% } %>
	<br><br><br>
	<a href = "employee?action=showAvailableProjects&id=<%= employee.getId() %>"><button>Assign employees</button></a>
	...
	<br><br><br>
	<% 
        List<Project> availableProjects = (List<Project>)request.getAttribute("availableProjects");
        if (null != availableProjects) { %>
          ...
          <%  for (Project project : availableProjects) { %>
             <tr>
	            <td><%= project.getId() %></td>
	            <td><%= project.getName() %></td>
	            <td><a href = "employee?action=assign&projectId=<%= project.getId() %>&employeeId=<%= employee.getId() %>">
	                <button>Assign</button></a></td>
	        </tr>
    <% } %>
    </table><br><br>
    <a href = "employee?action=view&id=<%= employee.getId() %>"><button>Cancel</button></a>
